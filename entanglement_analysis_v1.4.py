...
def ent_cmap(cor, ref = True, restricted = True, cut_off = 9.0, bb_buffer = 3, **kwargs):
    if print_framesummary: print(f'\nCMAP generator')
...
def get_crossing_residues(ent_info, coor):
    ...
    nc_list = [k for k,v in ent_info.items() if np.any(np.abs(v.round()) >= 1)]
...
if ref_path != 'nan':
    ...
    for nc, crossings in ref_crossings.items():
        ...
        #print(framenum, nc, gvals, crossings, ent_res)
